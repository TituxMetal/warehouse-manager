...
import { GetLocationsByAisleIdUseCase } from './GetLocationsByAisleId.uc'
...
describe('GetLocationsByAisleIdUseCase', () => {
  let useCase: GetLocationsByAisleIdUseCase
  ...
  beforeEach(() => {
    ...
    useCase = new GetLocationsByAisleIdUseCase(
      mockLocationRepository as unknown as ILocationRepository
    )
  })

  describe('execute', () => {
    it('should return locations for an aisle', async () => {
      const aisleId = 1
      const location1 = createTestLocationEntity({ id: 1, aisleId })
      const location2 = createTestLocationEntity({ id: 2, aisleId })
      mockLocationRepository.findByAisleId.mockResolvedValueOnce([location1, location2])

      const result = await useCase.execute(aisleId)

      expect(mockLocationRepository.findByAisleId).toHaveBeenCalledWith(aisleId)
      expect(result).toHaveLength(2)
    })

    it('should return empty array when no locations exist', async () => {
      mockLocationRepository.findByAisleId.mockResolvedValueOnce([])
    ...
    it('should handle repository errors', async () => {
      ...
      mockLocationRepository.findByAisleId.mockRejectedValueOnce(error)
