...
import { GetAvailableLocationsUseCase } from './GetAvailableLocations.uc'
...
describe('GetAvailableLocationsUseCase', () => {
  let useCase: GetAvailableLocationsUseCase
  ...
  beforeEach(() => {
    ...
    useCase = new GetAvailableLocationsUseCase(
  ...
  describe('execute', () => {
    it('should return available locations', async () => {
      const location1 = createTestLocationEntity({ id: 1, status: 'available' })
      const location2 = createTestLocationEntity({ id: 2, status: 'available' })
      mockLocationRepository.findAvailableLocations.mockResolvedValueOnce([location1, location2])

      const result = await useCase.execute()

      expect(mockLocationRepository.findAvailableLocations).toHaveBeenCalled()
      expect(result).toHaveLength(2)
    })

    it('should return empty array when no available locations exist', async () => {
      mockLocationRepository.findAvailableLocations.mockResolvedValueOnce([])
    ...
    it('should handle repository errors', async () => {
      ...
      mockLocationRepository.findAvailableLocations.mockRejectedValueOnce(error)
